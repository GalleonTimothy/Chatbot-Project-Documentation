...
def main():
    ...
    if st.button("Ask"):
        if user_question:
            txt_data = read_txt('./docs/data.txt')
            pdf_data = read_pdf('./docs/PIA.pdf')
            combined_data = f"{txt_data}\n\n{pdf_data}"
            prompt = f"{combined_data} {user_question}"
            response = conversation(prompt)
            message = {'human': user_question, 'AI': response['response']}
            st.session_state.chat_history.append(message)
            st.write("BotLeg:", response['response'])
